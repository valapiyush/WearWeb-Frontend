...
const DashboardLayout = () => {
  ...
  useEffect(() => {
    const fetchDashboardStats = async () => {
      try {
        const sellerId = localStorage.getItem('id'); // Get seller ID from localStorage
        if (!sellerId) return;

        const res = await axios.get(`/dashboard-stats?sellerId=${sellerId}`);
        setStats(res.data.data);
      } catch (err) {
        console.error('Failed to fetch seller dashboard stats', err);
      }
    };
